...
harbor_admin_password=`cat ./inventory/hosts | grep harbor_admin_password | sed s/\'//g | sed s/\"//g | awk -F '='  '{print $2}'`
./harbor/sentry/prepare.sh $cluster_dir $harbar_vip $harbor_protocol $harbor_admin_password
